class ZCL_ZGWSD_ENVIO_REMESS_DPC definition
  public
...
protected section.
  ...
  methods ZI_REMESSARABBIT_CREATE_ENTITY
    importing
    ...
    exporting
      !ER_ENTITY type ZCL_ZGWSD_ENVIO_REMESS_MPC=>TS_ZI_REMESSARABBITMQTYPE
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZI_REMESSARABBIT_DELETE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZI_REMESSARABBIT_GET_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZI_REMESSARABBIT_GET_ENTITYSET
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    exporting
      !ET_ENTITYSET type ZCL_ZGWSD_ENVIO_REMESS_MPC=>TT_ZI_REMESSARABBITMQTYPE
      !ES_RESPONSE_CONTEXT type /IWBEP/IF_MGW_APPL_SRV_RUNTIME=>TY_S_MGW_RESPONSE_CONTEXT
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZI_REMESSARABBIT_UPDATE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
...
CLASS ZCL_ZGWSD_ENVIO_REMESS_DPC IMPLEMENTATION.
...
*&* This class has been generated on 22.02.2022 11:02:36 in client 500
*&*
*&*       WARNING--> NEVER MODIFY THIS CLASS <--WARNING
*&*   If you want to change the DPC implementation, use the
*&*   generated methods inside the DPC provider subclass - ZCL_ZGWSD_ENVIO_REMESS_DPC_EXT
*&-----------------------------------------------------------------------------------------------*

 DATA zi_remessarabbit_create_entity TYPE zcl_zgwsd_envio_remess_mpc=>ts_zi_remessarabbitmqtype.
...
*-------------------------------------------------------------------------*
*             EntitySet -  ZI_RemessaRabbitMQ
*-------------------------------------------------------------------------*
     WHEN 'ZI_RemessaRabbitMQ'.
*     Call the entity set generated method
    zi_remessarabbit_create_entity(
         EXPORTING iv_entity_name     = iv_entity_name
       	 ...
       	 IMPORTING er_entity          = zi_remessarabbit_create_entity
    ).
*     Send specific entity data to the caller interfaces
    copy_data_to_ref(
      EXPORTING
        is_data = zi_remessarabbit_create_entity
      CHANGING
        cr_data = er_entity
   ).
...
*-------------------------------------------------------------------------*
      when 'ZI_RemessaRabbitMQ'.
*     Call the entity set generated method
     zi_remessarabbit_delete_entity(
          EXPORTING iv_entity_name     = iv_entity_name
                    iv_entity_set_name = iv_entity_set_name
                    iv_source_name     = iv_source_name
                    it_key_tab         = it_key_tab
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA zi_remessarabbit_get_entity TYPE zcl_zgwsd_envio_remess_mpc=>ts_zi_remessarabbitmqtype.
 DATA lv_entityset_name TYPE string.
...
*     Call the entity set generated method
          zi_remessarabbit_get_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = zi_remessarabbit_get_entity
                         es_response_context = es_response_context
          ).

        IF zi_remessarabbit_get_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = zi_remessarabbit_get_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
*&*   generated methods inside the DPC provider subclass - ZCL_ZGWSD_ENVIO_REMESS_DPC_EXT
*&-----------------------------------------------------------------------------------------------*
 DATA zi_remessarabbit_get_entityset TYPE zcl_zgwsd_envio_remess_mpc=>tt_zi_remessarabbitmqtype.
 DATA lv_entityset_name TYPE string.
...
*-------------------------------------------------------------------------*
   WHEN 'ZI_RemessaRabbitMQ'.
*     Call the entity set generated method
      zi_remessarabbit_get_entityset(
        EXPORTING
         iv_entity_name = iv_entity_name
         iv_entity_set_name = iv_entity_set_name
         iv_source_name = iv_source_name
         ...
         io_tech_request_context = io_tech_request_context
       IMPORTING
         et_entityset = zi_remessarabbit_get_entityset
         es_response_context = es_response_context
       ).
*     Send specific entity data to the caller interface
      copy_data_to_ref(
        EXPORTING
          is_data = zi_remessarabbit_get_entityset
        CHANGING
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA zi_remessarabbit_update_entity TYPE zcl_zgwsd_envio_remess_mpc=>ts_zi_remessarabbitmqtype.
 DATA lv_entityset_name TYPE string.
 DATA lr_entity TYPE REF TO data. "#EC NEEDED
...
*-------------------------------------------------------------------------*
      WHEN 'ZI_RemessaRabbitMQ'.
*     Call the entity set generated method
          zi_remessarabbit_update_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         iv_entity_set_name = iv_entity_set_name
                         iv_source_name     = iv_source_name
                         io_data_provider   = io_data_provider
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = zi_remessarabbit_update_entity
          ).
       IF zi_remessarabbit_update_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = zi_remessarabbit_update_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
* Unit test injection
  ...
  method IF_SADL_GW_DPC_UTIL~GET_DPC.
    TYPES ty_zi_remessarabbitmq_1 TYPE zi_remessarabbitmq ##NEEDED. " reference for where-used list

    DATA(lv_sadl_xml) =
               |<?xml version="1.0" encoding="utf-16"?>| &
               |<sadl:definition xmlns:sadl="http://sap.com/sap.nw.f.sadl" syntaxVersion="" >| &
               | <sadl:dataSource type="CDS" name="ZI_REMESSARABBITMQ" binding="ZI_REMESSARABBITMQ" />| &
               |<sadl:resultSet>| &
               |<sadl:structure name="ZI_RemessaRabbitMQ" dataSource="ZI_REMESSARABBITMQ" maxEditMode="RO" exposure="TRUE" >| &
    ...
    ro_dpc = cl_sadl_gw_dpc_factory=>create_for_sadl( iv_sadl_xml   = lv_sadl_xml
               iv_timestamp         = 20220222140234
               iv_uuid              = 'ZGWSD_ENVIO_REMESSARABBITMQ'
               io_context           = me->mo_context ).
  endmethod.


  method ZI_REMESSARABBIT_CREATE_ENTITY.
  ...
  method ZI_REMESSARABBIT_UPDATE_ENTITY.
